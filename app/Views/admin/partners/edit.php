...
<div class="form-card">
    <form action="/admin/partners/edit/<?= $partner['id'] ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
        <?= csrf_field() ?>
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" required value="<?= htmlspecialchars($partner['name']) ?>" class="form-control">
                <span class="help-text">Name of the collaborating institute or corporate sponsor.</span>
            </div>

            <!-- Partner Logo -->
            <div class="form-group">
                <label for="logo">Partner Logo Image (Leave empty to keep current)</label>
                <input type="file" id="logo" name="logo" accept="image/*" class="form-control">
                <span class="help-text">Current Logo: <code><?= htmlspecialchars($partner['logo']) ?></code></span>
            </div>

            <!-- Website -->
            <div class="form-group">
                <label for="website">Website URL</label>
                <input type="url" id="website" name="website" value="<?= htmlspecialchars($partner['website'] ?? '') ?>" class="form-control">
            </div>

            <!-- Category -->
            <div class="form-group">
                <label for="category">Category</label>
                <input type="text" id="category" name="category" value="<?= htmlspecialchars($partner['category'] ?? '') ?>" class="form-control">
            </div>

            <!-- Collaboration Start Date -->
            <div class="form-group">
                <label for="collaboration_start">Collaboration Start Date</label>
                <input type="date" id="collaboration_start" name="collaboration_start" value="<?= $partner['collaboration_start'] ? date('Y-m-d', strtotime($partner['collaboration_start'])) : '' ?>" class="form-control">
            </div>

            <!-- Collaboration End Date -->
            <div class="form-group">
                <label for="collaboration_end">Collaboration End Date</label>
                <input type="date" id="collaboration_end" name="collaboration_end" value="<?= $partner['collaboration_end'] ? date('Y-m-d', strtotime($partner['collaboration_end'])) : '' ?>" class="form-control">
            ...
            <div class="form-group">
                ...
                <input type="number" id="display_order" name="display_order" min="0" value="<?= intval($partner['display_order']) ?>" class="form-control">
            ...
            <div class="form-group" style="display: flex; flex-direction: column; gap: 1rem; padding-top: 1.5rem;">
                <label class="switch-container">
                    <input type="checkbox" id="featured" name="featured" value="1" <?= $partner['featured'] ? 'checked' : '' ?>>
                    <span class="switch-slider"></span>
                    <span style="font-weight: 600; color: var(--text-main); font-size: 0.95rem;">Featured Associate</span>
                </label>
                <label class="switch-container">
                    <input type="checkbox" id="active" name="active" value="1" <?= $partner['active'] ? 'checked' : '' ?>>
                    <span class="switch-slider"></span>
            ...
            <div class="form-group full-width">
                ...
                <textarea id="description" name="description" rows="3" class="form-control"><?= htmlspecialchars($partner['description'] ?? '') ?></textarea>
            </div>
        </div>
